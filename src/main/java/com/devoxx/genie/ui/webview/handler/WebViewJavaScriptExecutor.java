...
public class WebViewJavaScriptExecutor {
    ...
    public WebViewJavaScriptExecutor(JBCefBrowser browser) {
        ...
        if (browser != null) {
            log.debug("WebViewJavaScriptExecutor initialized for browser");
            startPendingExecutionProcessor();
        } else {
            log.warn("WebViewJavaScriptExecutor initialized with null browser");
        }
    }
    ...
    public void setLoaded(boolean loaded) {
        ...
        if (wasLoaded != loaded) {
            log.info("Browser loaded state changed: {} -> {}", wasLoaded, loaded);
            if (loaded && !pendingExecutions.isEmpty()) {
                log.info("Browser became loaded, processing {} pending executions", pendingExecutions.size());
    ...
    public void executeJavaScript(String script) {
        if (script == null || script.trim().isEmpty()) {
            log.warn("Attempted to execute null or empty JavaScript");
            return;
        }
    ...
    private void queuePendingExecution(String script) {
        if (pendingExecutions.size() >= MAX_PENDING_EXECUTIONS) {
            ...
            if (removed != null) {
                log.warn("Pending execution queue full, dropped oldest execution");
            }
        }

        pendingExecutions.offer(new PendingJSExecution(script, 0));
        log.debug("Queued JavaScript execution, {} pending", pendingExecutions.size());
    }
    ...
    private void startPendingExecutionProcessor() {
        ...
        log.debug("Started pending execution processor with 2s interval");
    }
    ...
    public void executeJavaScriptWithRetry(String script, int maxRetries, int retryDelayMs) {
        if (script == null || script.trim().isEmpty()) {
            log.warn("Attempted to execute null or empty JavaScript with retry");
            return;
        }

        log.debug("Executing JavaScript with retry (max retries: {}, delay: {}ms): {}",
    ...
    private void attemptJavaScriptExecution(String script, int maxRetries, int retryDelayMs, int currentAttempt) {
        if (!JCEFChecker.isJCEFAvailable() || browser == null) {
            log.debug("Cannot execute JavaScript - JCEF not available or browser is null");
            return;
        }
        ...
        ApplicationManager.getApplication().invokeLater(() -> {
            try {
                if (isLoaded.get()) {
                    ...
                    log.debug("Browser not ready, scheduling retry {} in {}ms", currentAttempt + 1, retryDelayMs);
                    Timer retryTimer = new Timer(retryDelayMs, e ->
                        attemptJavaScriptExecution(script, maxRetries, retryDelayMs, currentAttempt + 1));
                    retryTimer.setRepeats(false);
                    retryTimer.start();
                } else {
                    log.warn("JavaScript execution failed after {} attempts - browser not ready", maxRetries + 1);
                }
            } catch (Exception e) {
                if (currentAttempt < maxRetries) {
                    log.debug("JavaScript execution failed on attempt {}, scheduling retry: {}",
                    ...
                    log.error("JavaScript execution failed after {} attempts: {}", maxRetries + 1, e.getMessage());
                }
            }
        });
    }
    ...
    public void forcePendingExecution() {
        log.info("Force processing pending executions - {} queued", pendingExecutions.size());
        processPendingExecutions();
    }
    ...
    public void clearPendingExecutions() {
        ...
        log.info("Cleared {} pending JavaScript executions", cleared);
    }
     ...
     */
    public void dispose() {
        log.debug("Disposing WebViewJavaScriptExecutor");

        if (pendingExecutionTimer != null) {
            pendingExecutionTimer.stop();
            pendingExecutionTimer = null;
            log.debug("Pending execution timer stopped");
        }

        int pendingCount = pendingExecutions.size();
        pendingExecutions.clear();

        log.info("WebViewJavaScriptExecutor disposed - cleared {} pending executions", pendingCount);
    }
}
