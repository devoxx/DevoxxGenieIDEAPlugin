...
public final class TipService {
    ...
    private static final List<Tip> FALLBACK_TIPS = List.of(
            ...
            new Tip("Run fully local with Ollama, LMStudio, GPT4All or Llama.cpp — no API key needed.", 2),
            new Tip("Create your own slash commands in Settings → Commands.", 2),
            new Tip("Type /find to search your indexed project semantically with RAG.", 1),
            new Tip("Type /search to augment your prompt with a live web search.", 1),
            new Tip("Enable web search (Google or Tavily) to ground answers in live results.", 1),
            new Tip("Watch the live token count and cost estimate as you build a prompt.", 1),
            new Tip("Tune chat memory size in Settings to control how much history the LLM sees.", 1),
            new Tip("Set your own submit and newline shortcuts in Settings.", 1),
            new Tip("Enable Inline Completion for AI code suggestions as you type.", 1),
            new Tip("Enable Security Scanning — Gitleaks, OpenGrep and Trivy run as agent tools.", 1),
            new Tip("Define tasks in Backlog.md and let the Agent implement them from the Spec Browser.", 1),
            new Tip("Run prompts through Claude Code, Gemini CLI or Copilot with CLI/ACP Runners.", 1),
            new Tip("Drop a SKILL.md file into your skills folder to extend the agent in Agent mode.", 1),
            new Tip("Automate agents on events — Code Review on commit, Debug on test failure.", 1),
            new Tip("Your conversations are saved — reopen or remove past chats from history.", 1),
            ...
            new Tip("Customize the system prompt in Settings → System Prompt & Personas.", 1),
            new Tip("Set up personas in Settings → System Prompt & Personas, then switch role from the dropdown.", 2),
