...
public class PromptExecutionService implements ChatChangeListener {
    ...
    private final ChatMemoryService chatMemoryService = ChatMemoryService.getInstance();
    ...
    public @NotNull CompletableFuture<Optional<AiMessage>> executeQuery(@NotNull ChatMessageContext chatMessageContext) {
        ...
        try {
            ...
            if (chatMemoryService.isEmpty()) {
                chatMemoryService.add(createSystemMessage(chatMessageContext));
            }

            createUserMessage(chatMessageContext);

            chatMemoryService.add(chatMessageContext.getUserMessage());
    ...
    private @NotNull Optional<AiMessage> processChatMessage(ChatMessageContext chatMessageContext) {
        try {
            ...
            List<ChatMessage> messages = chatMemoryService.messages();
            Response<AiMessage> response = chatLanguageModel.generate(messages);
            chatMemoryService.add(response.content());
    ...
    public void removeMessagePair(ChatMessageContext chatMessageContext) {
        ...
        messages.forEach(chatMemoryService::add);
    }
}
