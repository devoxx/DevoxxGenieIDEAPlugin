...
- **🛡️ [Command Blacklist](https://genie.devoxx.com/docs/features/agent-mode#command-blacklist)**: Tell the agent which shell commands it may never run unsupervised. Destructive commands like `git reset --hard` or `rm -rf` are caught even inside compound commands (`cd x && git reset --hard`) and with reordered flags (`-rf` also matches `-fr`), and either force the approval dialog or are blocked outright — even when write approvals are auto-approved.
...
- **☁️ [Cloudflare AI Gateway](https://genie.devoxx.com/docs/llm-providers/cloud-models#cloudflare-ai-gateway)**: Point DevoxxGenie at your Cloudflare AI Gateway and use every model behind it with a single Cloudflare token (BYOK) — your provider keys stay in Cloudflare, and you get caching, rate limiting, spend controls and per-request analytics for free.
